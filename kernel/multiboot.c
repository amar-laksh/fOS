...
multiboot_check(multiboot_info_t* mbd, unsigned int magic)
{
  ...
  if (CHECK_FLAG(mbd->flags, 6)) {
    ...
    while ((unsigned long)mmap < mbd->mmap_addr + mbd->mmap_length) {
      ...
      mmap =
        (memory_map_t*)((unsigned long)mmap + mmap->size + sizeof(mmap->size));
    }
    total_mem_low /= 1024;
    total_mem_low /= 1024;
    total_mem_low += 1;
  ...
  if (CHECK_FLAG(mbd->flags, 3)) {
    ...
    for (i = 0; i <= mbd->mods_count; i += 2) {
      ...
      memory_t.modules[n].module_end =
        (uint32_t)ramdisk + (module_end - module_start);
  ...
  if (CHECK_FLAG(mbd->flags, 5)) {
    elf_section_header_table_t* multiboot_elf_sec = &(mbd->u.elf_sec);
