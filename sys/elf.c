...
void
elf_parse_header(void* ptr_to_header)
{
  ...
  int status = *(elf_identification) == ELFMAG0
                 ? *(elf_identification + 1) == ELFMAG1
                     ? *(elf_identification + 2) == ELFMAG2
                         ? *(elf_identification + 3) == ELFMAG3 ? 1 : 0
                         : 0
                     : 0
                 : 0;
...
ELF32_Shdr
elf_get_section(ELF32_Shdr shdrs_array[], const char* section_name)
{
  ELF32_Shdr ptr;
...
void
elf_dump_info(ELF32_Shdr elf32_shdr[], ELF32_Phdr elf32_phdr[])
{
  sprintf("\nELF information:\n");
  sprintf("\nELF Header\n");
  ...
  sprintf("\nELF Program Headers\n\n");
  sprintf("Type\t  Offset\t  VirtAddr\t  PhyAddr\t  FileSiz\t  MemSiz\t  Flg\t "
          " Align\n");
...
elf_install()
{
  ...
  for (int i = 0; i < elf32_header.elf_phnum; ++i) {
    void* ptr_to_header =
      (memory_t.modules[0].module_start +
       (elf32_header.elf_phoff + (elf32_header.elf_phentsize * i)));
  ...
  for (int i = 0; i < elf32_header.elf_shnum; ++i) {
    void* ptr_to_header =
      (memory_t.modules[0].module_start +
       (elf32_header.elf_shoff + (elf32_header.elf_shentsize * i)));
  ...
  void* _shstrtable = memory_t.modules[0].module_start +
                      shdrs_array[elf32_header.elf_shstrndx].sh_offset;
