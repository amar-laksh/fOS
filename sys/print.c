...
static void
print_dec(unsigned int value, unsigned int width, char* buf, int* ptr);

static void
print_hex(unsigned int value, unsigned int width, char* buf, int* ptr);
  ...
  for (; i < len && fmt[i]; ++i) {
    ...
    switch (fmt[i]) {
      ...
      case 'x': /* Hexadecimal number */
        print_hex(
          (unsigned long)va_arg(args, unsigned long), arg_width, buf, &ptr);
        break;
