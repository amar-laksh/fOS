...
first_push_ball()
{
  ...
  while (!((ball.dx >= -2 && ball.dx <= 2) && (ball.dy >= -2 && ball.dy <= 2) &&
           (ball.dx != 0))) {
...
void
pong_time_handler(irq_handler_t* handler)
{
  cycles++;
  game_cycles++;
    ...
    if (ball.playing == -13) {
      if ((ball.y <= 0 || ball.y >= 25)) {
        ...
        if ((ball.dx <= -1 && ball.dy <= -1) ||
            (ball.dx <= -1 && ball.dy >= 0) ||
            (ball.dx >= 0 && ball.dy <= -1) || (ball.dx >= 0 && ball.dy >= 0)) {
          ball.dy *= -1;
        }
      } else if (CHECK_AHEAD_CELL(ball.y, ball.x, ball.dy, ball.dx) == 'I') {
        if ((ball.dx >= 0 && ball.dy >= 0) || (ball.dx >= 0 && ball.dy <= -1))
          ball.dx *= -1;
        else
          ball.dx *= -1;
      }
...
void
pong_keyboard_handler(irq_handler_t* handler)
{
