...
void
itoa(uint64_t i, uint32_t base, char* buf);

int8_t
equals(char* first, char* second);

char*
str_tok(char* str, const char* delim);

//------------------------------------------------CONV.H

void
itoi(uint64_t i, uint32_t base, uint64_t* buf);
...
unsigned long long
multiboot_check(multiboot_info_t* mbd, unsigned int magic);
#endif /* ! ASM */
...
void
mm_init(uint32_t end, unsigned long total_mem);
void
mm_print_out();
...
void
irq_install_handler(int irq, irq_handler_t handler);

void
irq_uninstall_handler(int);
...
uint64_t
get_control_register(int number);
...
int32_t
get_column(uint32_t p);

int32_t
get_point(uint32_t r, uint32_t c);
...
char
get_char_cell(int r, int c);

void
draw_char(uint32_t p, char ch, uint8_t fg, uint8_t bg);

int32_t
draw_str(char string[], int32_t r, int32_t c);

void
write_char(char ascii, int fg, int bg);

void
write_str(char* string, int fg, int bg);
...
static inline void
outs(uint16_t port, uint16_t val)
{
...
uint32_t
play_note(char note, int octave, int time);
// Play sound using built in speaker
void
play_sound(uint32_t nFrequence);
...
#define CHECK_RANGE_FOR_BIT(begin, count, reg, what)                           \
...
static inline void
cpuid(uint32_t reg, uint32_t* eax, uint32_t* ebx, uint32_t* ecx, uint32_t* edx)
{
...
void
get_cpuid_string(int code, char* where);
...
void
read_text(char* ptr, unsigned int count);

void
reboot();

void
exec_cmd(int n, char* buff[5]);
...
void
process_controls(unsigned char scancode);
